...
sp <- function(prior, data=NULL, posterior=NULL){
    ...
    for(i in 1:ncol(prior)){
      ...
      dii <- paste0(di, ",", i)
      if(is.null(posterior)) main <- bquote(theta[.(i)])
      plot(x, 
           type='n',main=main,
...
overall <- function(prior, x = seq(0,1,length.out=1000), rsn=1000){
  priors <- sapply(1:ncol(prior), function(i) dbeta(x, shape1 = prior['alpha',i], shape2 = prior['beta',i]))
...
oplot <- function(thetao, data=T){
   ...
   main <- ifelse(data,"",bquote(theta))
   oc <- rgb(86, 35, 127, alpha=200, maxColorValue = 255)
   plot(x=seq(0,1,length.out=100),main=main,
        type='n',ylab='', xlab='', yaxt='n', yaxs='i', xaxt='n',xaxs='i',
        xlim=c(0,1), ylim=c(0,max(density(thetao)$y) * 2))
   hist(thetao, 
        add = TRUE,
...
data <- mapply(rbinom, n=100, size=1, prob=theta)
prior <- priorsi

dp <- t(sapply(seq(0,81,20), function(i){
  d <- data[i:(i+19),]
  sprintf("%.2f",apply(d, 2, sum) / nrow(d))
}))
```
...
cat("$$\\begin{array}{c|ccccc}")
cat("& ",paste0(paste0("\\theta_",1:5), collapse=" & "),"\\\\ \\hline")
...
\h How will the posterior distributions update if you receive the following data? Each column represents responses associated with a given subgroup. Each row represents 20 responses to invitations received over time.
```{r}
#| echo: false
#| results: asis

cat("$$y = \\left[\\begin{array}{ccccc}")
